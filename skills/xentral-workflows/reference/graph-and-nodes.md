...
| `in-erp` | ERP event (order created, document cancelled …) — **must be activated**, see the node catalog | `trigger-erp-event` |
...
## Calling a workflow via API (webhook request/response)

A `trigger-webhook` workflow is a request→response endpoint. Shape it as **trigger → processing → an `output` node** (the `output` node's `value` becomes the response; see the node catalog below).

1. **Read the input.** The request arrives as a fixed envelope: the POST body is `{{ trigger.body.<field> }}` (nested under `body`, not flat), plus `{{ trigger.query.<key> }}`, `{{ trigger.headers.<key> }}`, `{{ trigger.method }}`.
2. **Activate it.** A webhook is unreachable until a **Share** (share_id + optional token) exists. Mint it with the `xentral_workflows` tool (`action='share'`, `share_op='save'` → returns the `public_url`) or in the designer's webhook inspector.
3. **Call it.** `POST /wh/{share_id}?token=…`. With `?wait=1` the call is **synchronous** and returns `{status, run_id, output}` (`output` = the `output` node's value); without it the endpoint returns `202 {accepted, run_id}` immediately (fire-and-forget) and the run continues in the background.
4. **Test it.** The input comes from the *caller*, not an ERP read, so the "probe a source node against live data" loop does not apply. Exercise the whole graph with `xentral_workflow_debug` `run_workflow(trigger_payload=…)`, passing the same envelope shape — otherwise `{{ trigger.* }}` is empty and the run is not representative.
...
1. **Template fields** (`loop.items`, `loop.rollupValue`, `worklist.seed`/`enqueueFrom`/`rollupValue`, `expression.expression`, `transform.source`, `call-workflow.input`/`cacheKey`, `state.*.value`, `agent.prompt`, `http-request.url`/`body`/`headers`, `output.value`): use a `{{ name.path }}` hole, where `name` is the node's **variable name** (from `varName`, see above).
   - **Reserved namespaces:** `{{ state.<key>.<path> }}` reads the run-wide `state` dict (written by the `state` node); `{{ trigger.<key> }}` reads the run's input. The **shape of the trigger input depends on the trigger type** — it is not always flat: a `trigger-webhook` delivers `{ method, query, headers, body }` (so the POST body is `{{ trigger.body.<field> }}`), a `trigger-integration-event` delivers `{{ trigger.data.<field> }}`, and a `call-workflow` child receives the passed `input` map read flat as `{{ trigger.<key> }}`. Neither namespace collides with node variable names.
...
3. **Resource-node param values** (`business-entity` `params.<op>.{path,query,body}`): these do **NOT** understand `{{ }}`. A plain value is a literal; to inject an upstream value use a **binding object**:
   ```jsonc
...
- `trigger-webhook` — external HTTP endpoint. Config: `{ path, method }`. The incoming request reaches the graph as a **fixed envelope** — read its parts with `{{ }}` holes: `{{ trigger.body.<field> }}` is the request body (parsed JSON, so a POST of `{"name":"Ada"}` is `{{ trigger.body.name }}` — the body is nested under `body`, **NOT** flat under `trigger`), `{{ trigger.query.<key> }}` the URL query, `{{ trigger.headers.<key> }}` the (lowercased) headers, `{{ trigger.method }}` the verb. A webhook workflow is only reachable once a Share is minted — see **Calling a workflow via API** below.
- `trigger-erp-event` — an event from the instance's ERP core. Config: `{ event }`. **Saving the node subscribes NOTHING** — the subscription is registered/removed via `PUT .../workflows/{id}/erp-event`, or `xentral_workflows action='erp_event' erp_event_op='activate'` (the editor's inspector does this with its "Activate" button). Until then the ERP pushes nothing and the workflow never fires, while `validate` and `check` both look clean — `check` reports `erp_event_not_registered` for exactly this. The event id is the ERP's own, e.g. `"com.xentral.salesOrder.created.v1"` on a Xentral core or `"com.weclapp.salesOrder.updated.v1"` on a weclapp one — dot-segmented with a lowercase-first entity segment, **not** `"sales_order.created"`. Enumerate the valid ids with **`xentral_erp_core action='events'`** — the catalog is a core capability and lives on the core tool; this trigger only subscribes one of them. It is per-installation and per-version (~162 ids on a current Xentral), so never hardcode an id you have not seen in that response; an unknown id is refused at activation. The same call answers whether the active ERP core can push events at all (`core.supportsEvents: false` with an empty list) — Odoo and the Postgres core cannot, and there is no point wiring this trigger on them. The weclapp cores CAN: weclapp has its own webhook API, so ask for its catalogue the same way rather than assuming Xentral ids. Three differences matter there, all measured against a live weclapp: its ids are derived from the entity roster (entity x created/updated/deleted) rather than served, so an id it rejects fails at activation; a weclapp delivery carries only `{entityId, entityName, type}` and no record at all, so a workflow that needs field values must fetch by id; and one change fires exactly ONE event, unlike Xentral below. Event data is read downstream as `{{ trigger.body.type }}` and `{{ trigger.body.body.<field> }}`: the receiver wraps the ERP envelope in the same fixed `{method, headers, query, body}` shape the webhook trigger uses, so the vendor envelope sits nested under `body`, **NOT** flat under `trigger`. Note that on Xentral one business action can emit several events (creating one sales order fires `salesOrder.created`, `salesOrderPosition.created` and a protocol event), so subscribe to the one you mean; on weclapp it is one event per change.
...
- `xentral-api` — **not available for authoring.** Every Xentral read and write goes through `business-entity`, so a graph carrying this node is rejected at save. If the entity core cannot express what you need, say so and stop: the gap is reported automatically so the core can gain the capability, which is the only fix that helps the next tenant too.
...
- `human-task` — hand off to a human; endpoint node (no output). Config: `{ mode: 'task'|'job'|'email', … }` — `task` = operational decision item, `job` = agent job, `email` = Xentral correspondence email. Free-text fields support `{{ ref }}`.
- `output` — terminal node (no outgoing edges) that sets the workflow's **return value**. Config: `{ title, value }` where `value` is a template field — point it at an upstream result, e.g. `value: "{{ greeting }}"`. For a synchronous webhook call (`?wait=1`) this becomes the HTTP response body; for a `call-workflow` parent it is what `{{ <varName> }}` yields in the parent. A workflow that answers a caller (webhook / form) needs one; a fire-and-forget automation does not.
- `code` — inline Python snippet. Config: `{ title?, description, algorithm?, code }`. For data transforms and genuine program logic; upstream boxes are available as Python locals (their variable names). `description` is required, and `algorithm` is required once the logic contains loops/branches/aggregation. The Xentral helper `business_entity_op(entity_key, op, params)` is in scope too — use it for **data-dependent** fetches inside the code. Raw helpers (`xentral_op`, `xentral_request`) and bare `requests` are rejected at save (see "Fetching data vs. computing"). **Runs in a sandbox — only a safe subset of Python is allowed; see "Code node — what's allowed" before writing one** (notably: no `lambda`, no `import`).
...
- **ERP action** — authored as the `integration-action` node with `tool_id: 'xentral_erp'` (shown as the **ERP action / ERP-Aktion** picker in the inspector; there is **no** standalone `xentral-action` node type — a graph with `typeId: 'xentral-action'` is rejected as `unknown_type`). Pick ONE Xentral ERP action from the live catalog (`GET /instances/{lid}/xentral-actions?surface=workflow`, grouped by category: sales, shipping, warehouse, documents, communication, …) and fill its params (rendered from the action's `input_schema`; values are template fields, `{{ ref }}` supported). Config: `{ tool_id: 'xentral_erp', action, actionLabel, inputSchema?, preconditions?, params }`. It runs against the SAME registry (`thirdpartytools/xentral` `XentralMCPService`) that agents, MCP and Studio use. New actions (e.g. `email_send`, `print_document`, `stock_put_away`, `stock_retrieve`) appear here automatically without touching the node. Batch by feeding a `loop` into it. **Carrier/shipping labels are NOT in here — use the dedicated `shiplabel` node below.**
- `shiplabel` (palette group "Integrations" → **Paketmarke / Shipping label**) — create a **carrier-direct** shipping label (DHL, DPD, UPS, FedEx, GLS, Sendcloud, Shipcloud, …) via the standalone shiplabel service, independent of Xentral's own document flow. Config: `{ carrier, ... }` — pick a `carrier` and fill the shipment form (sender / recipient / parcel; values are template fields, `{{ ref }}` supported). Execution RPCs into `POST /instances/{lid}/shiplabel/create-label`; the result (normalized carrier response incl. the base64 label PDF + tracking number) is available under the node's variable name. **Needs carrier credentials** (`SHIPLABEL_<KEY>` env / inline config; carrier sandbox available for demos) — the credential fields live **on the node's own form** (carrier-aware), so leave the user a go-live activation task pointing there (see "Always leave a go-live activation task" in `SKILL.md`). **This is the ONE way to create a carrier/parcel label in a workflow** (e.g. "after picking, create the DHL Paketmarke"): reach for it whenever a label is needed, and never model a DHL/carrier label as an `http-request`, ERP action or `code` call. There is no shipping-label ERP action.
- `integration-action` (palette group "Integrations") — run ONE action of a connected external integration (Gmail, Slack, …) via the provider-neutral registry. Pick from the live catalog (`GET /instances/{lid}/integration-actions`, grouped by `tool_id`; only connected integrations appear) and fill its params (template fields, `{{ ref }}` supported). Config: `{ tool_id, action, actionLabel, connection_id, connectionLabel, paramDefs, params }`. The inspector auto-pins a connection into `connection_id`; the pin matters: unattended runs (schedule, webhook, integration event) execute with a service identity that owns no connections, so only a pinned connection — any connection of the instance — resolves there. Execution RPCs into `POST /instances/{lid}/integration-actions/{tool_id}/{action}`.
  - **Unconnected integrations:** the workflow catalog (`GET …/integration-actions`) lists only *connected* tools, but absence from it is **not** a veto — and **not** a licence to guess. Get the real `action` slug + params from the curated integration catalog first: `xentral_integrations` `list_actions` (`template_id=<tool_id>`) returns a toolkit's actions even when it isn't connected yet (e.g. Shopify's `SHOPIFY_GET_ORDERS_WITH_FILTERS`, `SHOPIFY_ADJUST_INVENTORY_LEVEL`, `SHOPIFY_CREATE_FULFILLMENT` — not a guessed `list_orders`). Author the node with that real slug, leave `connection_id` empty (unpinned), fill `params` with placeholders, and mark it **provisional**. Invent a best-guess slug only if the curated catalog has nothing for that tool. Tell the user the node needs the integration connected and a connection pinned in the inspector before the workflow runs; connecting is a separate, later step — never block building the shape on it. Leave a go-live activation task pointing at **Integrationen / Integrations** (`/app/integrations`) where the connect happens (see "Always leave a go-live activation task" in `SKILL.md`). (Same principle as the rest of the write path: don't refuse what the designer UI would let a human do.)
