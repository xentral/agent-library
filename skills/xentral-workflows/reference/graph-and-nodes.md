...
- `trigger-erp-event` — an event from the instance's ERP core. Config: `{ event }`. **Saving the node subscribes NOTHING** — the subscription is registered/removed via `PUT .../workflows/{id}/erp-event`, or `xentral_workflows action='erp_event' erp_event_op='activate'` (the editor's inspector does this with its "Activate" button). Until then the ERP pushes nothing and the workflow never fires, while `validate` and `check` both look clean — `check` reports `erp_event_not_registered` for exactly this. The event id is the ERP's own, e.g. `"com.xentral.salesOrder.created.v1"` — dot-segmented with a lowercase-first entity segment, **not** `"sales_order.created"`. Enumerate the valid ids with **`xentral_erp_core action='events'`** — the catalog is a core capability and lives on the core tool; this trigger only subscribes one of them. It is per-installation and per-version (~162 ids on a current Xentral), so never hardcode an id you have not seen in that response; an unknown id is refused at activation. The same call answers whether the active ERP core can push events at all (`core.supportsEvents: false` with an empty list) — Odoo, weclapp and the Postgres core cannot, and there is no point wiring this trigger on them. Event data is read downstream as `{{ trigger.body.type }}` and `{{ trigger.body.body.<field> }}`: the receiver wraps the ERP envelope in the same fixed `{method, headers, query, body}` shape the webhook trigger uses, so the vendor envelope sits nested under `body`, **NOT** flat under `trigger`. Note that one business action can emit several events (creating one sales order fires `salesOrder.created`, `salesOrderPosition.created` and a protocol event), so subscribe to the one you mean.
