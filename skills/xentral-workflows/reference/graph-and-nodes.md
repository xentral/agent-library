...
- `shiplabel` (palette group "ERP actions" → **Paketmarke / Shipping label**) — create a **carrier-direct** shipping label (DHL, DPD, UPS, FedEx, GLS, Sendcloud, Shipcloud, …) via the standalone shiplabel service, independent of Xentral's own document flow. Config: `{ carrier, ... }` — pick a `carrier` and fill the shipment form (sender / recipient / parcel; values are template fields, `{{ ref }}` supported). Execution RPCs into `POST /instances/{lid}/shiplabel/create-label`; the result (normalized carrier response incl. the base64 label PDF + tracking number) is available under the node's variable name. **Needs carrier credentials** (`SHIPLABEL_<KEY>` env / inline config; carrier sandbox available for demos). Reach for this node whenever a workflow must produce a real parcel label (e.g. "after picking, create the DHL Paketmarke"). Distinct from the `print_shipping_label` xentral-action, which triggers the label through Xentral's own delivery-note/CIS flow (`deliveryNotes/{id}/actions/printShippingLabel`) — use the action when Xentral already owns the shipment/carrier config, the `shiplabel` node when you want to call the carrier directly.
